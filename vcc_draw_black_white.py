...
bottom_hatch = "///////"  # 如果想要符号密集一些，可以在引号内多打一些斜杠
middle_hatch = "........"
top_hatch = ""
...
plt.bar(x, data_1, width=width, label="时延判决算法", color="white", edgecolor="black", linewidth=line_width, hatch=bottom_hatch)
plt.bar(x, data_2, width=width, bottom=data_1, label="联合判决算法", color="white", edgecolor="black", linewidth=line_width,  hatch=middle_hatch)
plt.bar(x, data_3, width=width, bottom=data_4, label="最多无环路径数", color="white", edgecolor="black", linewidth=line_width, hatch=top_hatch)
...
# 保存图片
# plt.savefig('paper_pic/black_white_pic/vcc.jpg', format='jpg', bbox_inches='tight', pad_inches=0)  # 紧凑型保存
# plt.savefig('paper_pic/black_white_pic/vcc.svg', format='svg', bbox_inches='tight', pad_inches=0)  # 保存成矢量图，方便修改
plt.show()